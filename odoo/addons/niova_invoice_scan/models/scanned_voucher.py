...
FILE_FORMATS = ('jpeg', 'jpg', 'png', 'tiff', 'pdf')
...
class ScannedVoucher(models.Model):
    ...
    def _set_attachment(self, file_name, file_data, mimetype=False):
        ...
        def encodebytes(file):
            if isinstance(file, str):
                file = file.encode('utf-8')
            return base64.encodebytes(file)
        
        try:
            attachment = {
                'name': file_name,
                'datas': encodebytes(file_data),
    ...
    def _get_mail_attachments(self, msg_dict):
        ...
        for attachment in msg_dict.get('attachments', []):
            # Sort out unsupported file format
            if attachment.fname.strip().split(".")[-1].lower()not in FILE_FORMATS:
                continue
